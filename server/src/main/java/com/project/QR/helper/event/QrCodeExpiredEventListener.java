...
@Component
@Slf4j
public class QrCodeExpiredEventListener {
  @Value("${mail.subject.qrCode.expired}")
  private String subject;
  private final EmailSender emailSender;

  public QrCodeExpiredEventListener(EmailSender emailSender) {
    this.emailSender = emailSender;
  }

  @Async
  @EventListener
  public void listen(QrCodeExpiredApplicationEvent event) throws Exception {
    try {
      Member member = event.getQrCode().getBusiness().getMember();
      QrCode qrCode = event.getQrCode();
      String[] to = new String[]{member.getEmail()};
      String message = "<h1>" + member.getName() + "님, 설정하신 QR 코드가 만료되기 하루 전입니다.</h1>\n" +
