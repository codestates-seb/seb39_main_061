...
function CreateQr() {
  ...
  const saveQRCode = async () => {
    const {
      ...
      "http://localhost:8080/api/v1/qr-code/reservation",
      body,
      {
        headers: {
    ...
    if (message === "CREATED") {
      ...
      QRCode.toDataURL(
        `${window.location.origin}/reservation/${data.qrCodeId}?target=${data.target}`,
        {
          width: 320,
          height: 320,
          color: {
        ...
        (err, url) => {
          ...
          const response = axios
            .post(
              `http://localhost:8080/api/v1/qr-code/reservation/${data.qrCodeId}/update`,
              formData,
              {
                headers: {
                  Authorization: `Bearer ${localStorage.getItem("token")}`,
